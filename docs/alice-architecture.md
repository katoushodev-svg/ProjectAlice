...
AI Capability BoundaryはADR-014により独立した`ai` Featureが所有する。
...
└── agent
        │
        ▼
ai Feature / Capability-specific Ports
├── Text Generation
...
Phase 1ではOpenAIを利用するText GenerationとInput Token Countingだけを実装する。Phase 2〜4用のPortまたはAdapterを先行実装しない。
...
6. Alice Core
   Alice人格・Conversation HistoryからContext生成
        ↓
7. TextGenerationProvider
        ↓
8. OpenAI Text Generation Adapter
        ↓
9. OpenAI API
        ↓
10. Alice Core
...
AI Capability Boundaryは独立した`ai` Featureとして配置する。ただしPhase 1で実装するのはConversation Text GenerationとInput Token Countingに必要な最小部分だけとする。
...
ADR-014により、Phase 0では次のPhase 1〜4 Architectureを設計対象とする。
...
Tools / External Services
      ↓
Phase 4
Agent + PC / Browser Operation + Voice

Phase 1は実装へ直接投入できる詳細度まで確定する。

Phase 2〜4はBoundary、主要Model、Port、Flow、SecurityおよびPermissionをPhase 0で確定し、変更可能性が高いProvider Model、SDK、Libraryまたは細かなParameterはImplementation-time Reconfirmationとする。

設計はPhase 1〜4を対象とするが、実装は各Phaseで必要な機能だけを追加する。
...
* Embedding Provider
* RAG Architecture
* Phase 2〜4でImplementation-time ReconfirmationとしたProvider / Model / SDK
