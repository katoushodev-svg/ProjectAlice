...
Flutter Frontend内部のArchitecture、State、API / SSE Client、画面状態およびPhase 1実装規則を定義する。
...
詳細はApprovedの`frontend-design.md`をSource of Truthとし、以下のFeature-based Structure + Layer Separationを作成する。
...
* REST API
* Alice Core
* 独立`ai` FeatureによるText Generation / Input Token Counting
* Conversation History
...
Phase 1ではconversation Featureと、ADR-014で正式採用した最小の`ai` Featureを実装する。将来用のmemory、tool、agent等の空Packageを先行作成しない。
...
Conversation FeatureはAI Infrastructureへ直接依存せず、独立`ai` FeatureのApplication Contractを利用する。

```text
conversation.application
        │
        ▼
ai.application.TextGenerationProvider
        ▲
        │ implements
ai.infrastructure OpenAI Adapter
```

Backend Runtime / Framework / Build ToolについてはADR-013、AI Capability BoundaryについてはADR-014および`ai-design.md`で決定済みとする。
...
Backendではconversation Featureと最小の`ai` Featureを実装する。

Conversation Historyはconversation Feature内で管理する。

Phase 1では独立したmemory Featureを作成しない。

Phase 1の`ai` FeatureにはText Generation、Input Token Counting、Provider-independent ModelおよびOpenAI Adapterだけを配置し、Phase 2〜4用Capabilityを先行実装しない。

agent/はPhase 4で必要になった時点で追加する。Phase 1ではDirectory、空PackageまたはBoilerplateを先行作成しない。
...
ADR-014に従い、Phase 0でPhase 1〜4のRepository / Module Boundaryを設計し、実体は必要なPhaseで追加する。

Phase 1
frontend + backend conversation / ai + Conversation History + DynamoDB
        ↓
Phase 2
...
Tools / External Services
        ↓
Phase 4
agent + PC / Browser Operation + Voice + Agent Capability
...
* Phase 1画面仕様のUser Approval（`frontend-ui-design.md`で設計）
...
Backend Runtime / Framework / Build ToolについてはADR-013およびdevelopment-environment.md、Phase 0設計範囲とAI Capability BoundaryについてはADR-014で決定済みとする。
