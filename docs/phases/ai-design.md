...
## Phase 4 Formal Integration Resolution — 2026-09-04

**Status:** Accepted / Integrated  
**Source:** AGENT4-001〜118, UI-XP-001, P4-FR-001〜121, P4-NFR-001〜024

This section is normative for Phase 4 and supersedes earlier Phase 4-adjacent text in this document where a conflict exists.

### AI / Agent / Execution Ownership

```text
conversation
└── User-facing orchestration / final response

ai
├── Conversation Generation
├── Tool Call Proposal
├── Agent Plan Proposal
└── Agent Replan Proposal

agent
├── Goal
├── Plan
├── AgentExecution orchestration
├── AgentAction
├── Observation
├── Evaluation
└── Replanning
...
└── Tool-specific Adapter / Validation
```

`AI Proposal ≠ Execution Authority`, `Agent Plan ≠ Permission`, and `Agent Action ≠ Approval`.
Provider output is a proposal. Schema-valid output is not sufficient execution authority.

### AgentExecution Lifecycle

Normative lifecycle states:

`PLANNING`, `RUNNING`, `WAITING_FOR_APPROVAL`, `NEEDS_USER_INPUT`, optional `PAUSED`,
`COMPLETED`, `FAILED`, `CANCELLED`, `UNKNOWN_OUTCOME`.

`DRAFT` and `AWAITING_APPROVAL` from earlier drafts are not the normative Phase 4 AgentExecution lifecycle names.

Lifecycle status and outcome are separate. Outcome is `SUCCESS`, `FAILURE`, `PARTIAL_SUCCESS`, or `UNKNOWN`.
For example, `COMPLETED + PARTIAL_SUCCESS` is valid.

### Risk Model

The normative common Phase 3/4 risk model is:

`LOW`, `MEDIUM`, `HIGH`, `CRITICAL`.

Earlier `READ_ONLY`, `REVERSIBLE_WRITE`, `IRREVERSIBLE_OR_HIGH_IMPACT` values are superseded as risk levels.
READ / WRITE / INTERACT / COMMIT / DELETE / EXECUTE / SYSTEM_CHANGE are operation kinds, not risk levels.

Risk is derived from operation kind plus target, scope breadth, reversibility, third-party effect,
privilege, sensitivity, ambiguity, and current-state confidence.
...
LLM / Provider may propose but may not authorize. External content may inform but may not authorize.
Generated text alone does not establish execution success; authoritative observation from the responsible
environment is required.

Existing safe defaults such as Maximum Plan Steps 20, Maximum Re-plans 3 and Concurrent Side-effecting
Steps 1 remain Phase 4 initial safe defaults. Relaxing them upward requires Architecture / Security review.
