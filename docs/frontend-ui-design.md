...
| Status | Proposed — Design in Progress |
...
Phase 1はPortraitを標準利用方向とする。Landscapeを明示的に禁止するか、最低限対応とするかはAccessibility / iOS設計で確定する。
...
| Link | Linkとして識別。外部遷移前の安全なInteractionは後続設計で確定 |
...
Partial Assistant ContentはCanonical Historyとして扱わない。Failure時に画面上へ残すか破棄するかはError / Retry設計で確定する。

### 9.8 Message Spacing Boundary

- 同一Roleが連続してもMessageごとの境界を維持する
- User / Aliceの交互出現を前提にLayoutを実装しない
- Message間Spacingの具体値はVisual Token設計で確定する
...
大量の細かなDeltaによる過剰な再描画を避ける具体的なUI Update間隔は、AI Streaming Contractと体感LatencyをTestしたうえで実装時に決定する。ただし受信Textを欠落・並べ替えしてはならない。
...
Static Mockupの形状をPixel単位で固定せず、同じIdentityを保ったまま緩やかに形が変わることを許容する。Animation仕様、描画方式およびPerformance Budgetは実装前のComponent詳細設計で確定する。
...
Alice CoreはMessage Historyより前面へ重ならない。Historyが増えた場合はMessage表示を優先し、Coreの占有領域を縮小またはScroll Contentの一部として扱う。正確なResponsive BehaviorはComponent詳細設計で確定するが、Conversation Historyの可読性を犠牲にしない。
...
- Phase 1で実装するAnimation範囲、Frame RateおよびBattery BudgetはComponent詳細設計で確定する
...
- Static Image、CustomPainter、Shader、Rive等の描画方式は未決定とし、AI Coding Assistantが独自にDependencyを追加しない
...
Core Base AssetはRepository管理下のLocal Assetとし、RuntimeでNetworkから取得しない。正式なFilename、Resolution、FormatおよびAsset生成手順はAsset Specificationで確定する。Assetが存在しない状態をRuntime Fallbackで隠さず、Build / Testで検出する。
...
Initial History LoadingはAliceが思考している状態ではないため`thinking`へMappingしない。Loading IndicatorとLabelをUI-008どおり別表示し、Alice Coreは非表示または`idle`のStatic表示とする。どちらを使用するかはScreen State Component設計で確定する。
...
- Reference Mockupの正確なRing本数、線幅およびGlow強度はAsset / Component Specificationで確定する
...
1. Phase 1 Alice Core Asset生成・検証
2. Frontend UI Design Review
...
| Next Topic | Phase 1 Alice Core Asset Generation and Validation |
