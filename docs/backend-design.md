...
主要Business Featureはconversationとする。AI Provider交換境界はADR-014に従い、独立した最小の`ai` Featureとして実装する。
...
Phase 1の`ai` FeatureにはText Generation、Input Token Counting、Provider-independent ModelおよびOpenAI Adapterだけを配置する。Phase 2〜4用のPort、Adapterまたは空Packageを先行作成しない。
...
SendMessageUseCase
        │
        ▼
ai.application.port.TextGenerationProvider
        ▲
        │ implements
ai.infrastructure.openai.OpenAiTextGenerationProvider
        │
        ▼
OpenAI API

AI Capability PortはProviderを利用するAlice Core側から参照する契約とし、独立した`ai` Featureが所有する。

具体的なOpenAI API Client、SDK、HTTP通信方式等はInfrastructure側へ隔離する。

⸻

18. AI Capability Responsibilities

AI Capability Boundaryでは概念的に以下を扱う。
...
Provider-independent AI Result / Text Delta
...
19. AI Capability Ownership Policy

Accepted Decision

ADR-014により、従来の`conversation` Feature所有方針を変更し、AI Capability Boundaryを独立した`ai` Featureへ昇格した。
...
│   ├── model/
│   └── port/
│       ├── TextGenerationProvider.java
│       └── InputTokenCounter.java
└── infrastructure/
    └── openai/

一つの巨大な`AiProvider` Interfaceは作成しない。Capabilityごとに小さなPortへ分割する。

Responsibility Boundary

`conversation` Featureは以下を所有する。

* Conversation Use Case
* Conversation History Selection
* Context構築
* Alice Personalityを含むPrompt構築
* AI ResultのConversationへの反映

`ai` Featureは以下を所有する。

* Provider-independent AI Request / Result Model
* AI実行制御Contract
* Text Generation Port
* Input Token Counting Port
* OpenAI Request / Stream / Error Mapping
...
`conversation`から`ai.infrastructure`へ直接依存してはならない。OpenAI SDK Object、EventまたはExceptionを`ai` Feature外へ公開してはならない。

Incremental Implementation Rule

独立`ai` Featureの採用は、将来機能をPhase 1へ先行実装する許可ではない。Personal Memory、Tool Calling、Agent、Voice用Capability Portは、それぞれのPhaseで具体的な利用契約が確定した時点で追加する。
...
SendMessageUseCase
      ...
      │               │
      ▼               ▼
Conversation       TextGenerationProvider
Repository             │
      │                 ▼
      ▼             OpenAI Adapter
DynamoDB Adapter       │
...
5. AI Request Context構築
        ↓
6. Input Token CountとTextGenerationProvider呼び出し
        ↓
7. OpenAI API呼び出し
        ↓
8. AI Response取得
...
* InputTokenCounter Port
* OpenAI Text Generation / Input Token Counting Adapter
...
Fake TextGenerationProvider / InputTokenCounter
...
本ドキュメントでは以下の詳細を直接確定せず、各Source of Truthへ委譲する。
...
Alice Core
    │
    ├── ai.application Capability Ports
    │       ▲
    │       │
    │   ai.infrastructure OpenAI Adapter
    │
    └── conversation.application ConversationRepository Port
            ▲
            │
        conversation.infrastructure DynamoDB Adapter

Phase 1ではconversation Featureと最小の`ai` Featureを実装する。
...
ai
├── Application Capability Ports / Models
