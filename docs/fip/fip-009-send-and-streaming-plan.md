...
- `stream.started`で`activeRequestId`を設定し、Pending User Messageを保存済みCanonical `userMessage`へ置換する
- Pending Original ContentとCanonical `userMessage.content`の完全一致、Role=`user`を確認する
- Completed / Failed ReplayでもCanonical User Messageを受理し、同じID / FieldはDeduplicateする
...
2. `stream.started`で受理済みのCanonical User MessageとCompletedの`userMessage`が同一ID / Fieldであることを確認する
...
- Started前Delta拒否
- `stream.started.userMessage`でPendingをCanonicalへ置換
- Pending Content / Canonical Content不一致を拒否
- Failed ReplayでもCanonical User Messageを保持
- Request ID不一致拒否
