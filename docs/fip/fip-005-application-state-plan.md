...
| `retryAfter` | `Duration?` | Backendが明示した再試行待機時間。`REQUEST_IN_PROGRESS`以外ではNull |
...
- `retryAfter`はHTTP `Retry-After`の有効なdelta-secondsだけから生成する
- `requestInProgress`では`retryAfter`を必須とし、それ以外のCategoryではNullとする
- `retryAfter`は自動Retryを許可する情報ではなく、FIP-010がUser Action可能時刻を判断するための情報とする
...
`sendStarted`は検証済み`OutgoingMessage`を一つ保持する。`streamStarted`は`requestId`とBackend保存済みCanonical User Messageを保持する。`assistantDeltaReceived`はTemporary Textだけを更新し、Canonical Message Listを変更しない。
...
- `streamStarted`で`activeRequestId`を設定し、Canonical User MessageをMessage ListへMergeする
- Canonical User ContentはPending SendのOriginal Contentと完全一致しなければならない
- Pending表示はCanonical User Messageへ置換するが、Logical Sendを表す`pendingSend`はTerminalまで保持する
- Replay時に同じCanonical User Messageが既に存在する場合はID / Field一致を確認してDeduplicateする
- Content不一致、同じIDでField不一致またはRole不一致は`protocolViolation`とする
...
- Completed後は`streamStarted`で受理済みのCanonical User MessageをID / Field一致で再検証し、Canonical Assistant Messageを追加してTemporary Textを破棄する
...
- Failed後も`streamStarted`で受理したCanonical User MessageはMessage Listへ残す
...
FIP-005 Draft Planning: Completed / User Confirmed
FIP-005 Implementation: Not Started
Cross-phase Review: Required
```

本Draftはユーザー確認済みであり、Draft PlanningをCompletedとして記録する。ソースコード実装には進まず、次にFIP-006 Visual FoundationのDraft実装計画書作成へ進む。
