...
| FIP-012 | Semantics、Dynamic Type、Reduce Motion、ContrastおよびGoldenの横断Gate |
...
FIP-011 Draft Planning: Completed / User Confirmed
FIP-011 Implementation: Not Started
Cross-phase Review: Required
```

本Draftはユーザー確認済みである。ソースコード実装には進まず、次にFIP-012 Accessibility and Visual GateのDraft実装計画書作成へ進む。
